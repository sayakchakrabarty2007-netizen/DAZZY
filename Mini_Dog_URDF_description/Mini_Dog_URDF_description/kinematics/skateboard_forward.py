#!/usr/bin/env python3
"""
Skateboard Forward Node — Synchronous Skate Impulse Gait
=========================================================

Locomotion mode that exploits the one-way ratchet wheels at the feet.
...
class SkateboardForwardNode(Node):
    ...
    def __init__(self):
        super().__init__('skateboard_forward_node')

        # Tunable parameters (Wide Stance Anti-Wheelie Skateboard)
        self.declare_parameter('stroke_amplitude', 0.15)    # Powerful stroke to generate enough momentum to glide
        self.declare_parameter('front_offset', -0.15)       # Front legs biased FORWARD (-) to widen support base
        self.declare_parameter('rear_offset', 0.15)         # Rear legs biased BACKWARD (+) to widen support base
        self.declare_parameter('reach_duration', 0.35)      # Fast swing forward (wheels roll freely)
        self.declare_parameter('push_duration', 0.25)       # Very fast, powerful backward push (wheels lock and push ground)
        ...
        self.get_logger().info('Skateboard Forward Node initialized (Zero-Accel Quintic Smoothing)')
    ...
    def _control_loop(self):
        ...
        front_angle = stroke + self.front_offset
        rear_angle = stroke + self.rear_offset
...
def main(args=None):
    ...
    try:
        ...
        node.get_logger().info('Shutting down Skateboard Forward Node...')
    finally:
